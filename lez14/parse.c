...
tcmdline parseLine(char *line)
{
  ...
  /* Nome comando, vettore dei parametri, numero parametri, nome fileIn, nome fileOut, nome fileErr, simbolo background. */
  ...
  while(result != NULL)
      ...
      else if(result[0] == '&')
	{
      ...
      else if(strCm.inFile[0] == '\0' && strCm.outFile[0] == '\0' && strCm.errFile[0] == '\0' && result[0] != '<' && result[0] != '>' && result[0] != '2' && result[1] != '>')
	{
	  if(n == 0)
